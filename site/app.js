(async () => {
  ...
  async function verify(){const message=$('verifyMessage');if(message)message.textContent='';const email=$('email')?.value.trim()||'',nickname=$('nickname')?.value.trim()||'';if(!email||!nickname){if(message)message.textContent=t('missingIdentity');return}if(!validEmail(email)){if(message)message.textContent=t('invalidEmail');return}if(!$('consent')?.checked){if(message)message.textContent=t('consentNeeded');return}busy($('verifyBtn'),true);try{const{response,data}=await call('verify',{email,nickname,privacy_consent:true,website:$('website')?.value||''});if(data.code==='ALREADY_SUBMITTED'){showDone(true);return}if(!response.ok||!data.ok){if(['WINNER_PARTIAL_MISMATCH','WINNER_NOT_LISTED','WINNER_MISMATCH','IDENTITY_MISMATCH'].includes(data.code)){if(message)message.textContent='';window.alert(errorText(data.code));$('email')?.focus();return}if(message)message.textContent=errorText(data.code);return}state.token=data.token||data.verificationToken||'';state.accountEmail=email;state.nickname=nickname;state.eventDate=data.eventDate||'';state.eventDateTba=data.eventDateTba===true;if(!state.token){if(message)message.textContent=t('network');return}if($('contactEmail')&&(!state.reauthPending||!$('contactEmail').value))$('contactEmail').value=email;state.reauthPending=false;setText('eventDate',state.eventDateTba?tbaText():state.eventDate);showStep2()}catch{if(message)message.textContent=t('network')}finally{busy($('verifyBtn'),false);verifyReady()}}
