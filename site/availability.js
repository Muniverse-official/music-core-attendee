...
function arm(delay){
  clearTimeout(timer);timer=null;nextDue=mono()+delay;
  if(active())timer=setTimeout(()=>{timer=null;void refresh()},Math.max(0,delay));
}
function pollDelay(){
...
document.addEventListener('click',e=>{
  const b=e.target instanceof Element?e.target.closest('#verifyBtn,#submitBtn'):null;
  if(b&&state()!=='OPEN'){e.preventDefault();e.stopImmediatePropagation();render();wake()}
},true);
...
document.addEventListener('visibilitychange',()=>{
  if(document.hidden){clearTimeout(timer);timer=null;clearTimeout(wakeTimer);wakeTimer=null}
  else wake();
});
