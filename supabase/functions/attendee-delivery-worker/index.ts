const REV='20260908-delivery-v3';
...
async function health(program:string,c:O){
  let status='unreachable',version:number|null=null,error='DELIVERY_CONFIG_MISSING';
  if(c.apps_script_url&&c.webhook_token){try{
    ...
    const ready=r.ok&&j.ok===true&&j.service==='muniverse-attendee-dispatcher'&&j.version>=9&&j.sheetMode==='round-tabs'&&j.deliveryState==='per-row-v1'&&j.textCells===true;
    status=ready?'contract_ready':'upgrade_required';error=ready?'':'APPS_SCRIPT_UPGRADE_REQUIRED';
  ...
  return{ready:status==='contract_ready',error};
}
async function deliver(row:O,c:O){
  ...
  const r=await fetch(c.apps_script_url,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({version:9,ts:String(Date.now()),nonce:crypto.randomUUID(),token:c.webhook_token,kind:row.program,payload:row.payload}),signal:AbortSignal.timeout(20000)});
  let j:O={};try{j=await r.json();}catch{}
  // Partial completion is meaningful even when the webhook reports ok:false.
  const sheet=r.ok&&j.sheetUpdated===true,email=r.ok&&j.emailSent===true,ok=r.ok&&j.ok===true&&sheet&&email;
  ...
  try{
    ...
    await Promise.all([...groups.values()].map(async items=>{
      for(const row of items){
        ...
        try{
          const d=await deliver(row,configs[row.program]);
