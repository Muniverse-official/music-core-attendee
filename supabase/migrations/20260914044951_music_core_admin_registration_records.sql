-- Music Core registrations remain in the administrator's per-round records.
-- FANS PICK retains its existing delivery workflow. Historical sheet data is untouched.
...
begin
  ...
  begin
    if p_program='music_core' then
      ...
      insert into public.music_core_audit_log(event,winner_id,ip_hash,metadata) values('submit_success',w.id,p_ip_hash,jsonb_build_object('round_id',r.id,'selection_type',v_selection_type,'delivery_queued',p_program='fans_pick','stateless_token',true));
    else
...
begin
  ...
  with picked as (
    select id from public.attendee_delivery_outbox
     where program='fans_pick' and status='pending' and next_attempt_at<=clock_timestamp()
     order by id
     limit p_limit
...
CREATE OR REPLACE FUNCTION public.attendee_delivery_is_current(p_id bigint)
 ...
 SET search_path TO 'pg_catalog', 'public'
AS $function$
  select exists(select 1 from public.attendee_delivery_outbox o where o.id=p_id and o.program='fans_pick' and o.status='processing'
...
begin
  if p_program not in ('music_core','fans_pick') then raise exception 'INVALID_PROGRAM';end if;
  if p_program='music_core' then raise exception 'DELIVERY_DISABLED';end if;
  perform pg_advisory_xact_lock(hashtextextended('attendee-round:'||p_program,0));
