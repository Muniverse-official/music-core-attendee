-- Reserve winners pre-register the same information, without confirming attendance.
alter table public.music_core_winners drop constraint music_core_reserve_not_submitted;
drop trigger music_core_primary_registration_guard on public.music_core_attendees;
drop function public.music_core_primary_registration_guard();
comment on column public.music_core_winners.selection_type is
  'primary: selected attendee; reserve: waiting list with advance registration, promoted on vacancy';
...
begin
  ...
  begin
    if p_program='music_core' then
      ...
      insert into public.music_core_audit_log(event,winner_id,ip_hash,metadata) values('submit_success',w.id,p_ip_hash,jsonb_build_object('round_id',r.id,'selection_type',v_selection_type,'delivery_queued',v_selection_type='primary','stateless_token',true));
    else
  ...
  if v_selection_type='primary' then
...
declare r public.attendee_rounds;src public.attendee_rounds;rid uuid;prev uuid;pre text;c jsonb;entry jsonb;wid uuid;d date;n integer=0;submitted boolean;selection text;outcome jsonb;begin
 ...
 if p_action='create_round' then
  ...
  if p_action='save_config' then
   ...
   -- A reserve registration is retained; deliver its existing information only on promotion.
   insert into public.attendee_delivery_outbox(program,winner_id,round_id,payload)
   select p_program,w.id,r.id,jsonb_build_object('event_date',w.event_date::text,'account_email',a.account_email,'muniverse_nickname',a.muniverse_nickname,'name',a.name,'nationality',a.nationality,'birth_date',a.birth_date::text,'phone',a.phone,'x_account',a.x_account,'contact_email',a.contact_email,'round_id',r.id,'round_title',r.title,'idempotency_key',p_program||':'||w.id::text)
   from public.music_core_winners w join public.music_core_attendees a on a.winner_id=w.id
   where w.id=wid and w.round_id=rid and w.submitted
   on conflict(program,winner_id) do nothing;
   outcome=jsonb_build_object('ok',true,'round_id',rid);
